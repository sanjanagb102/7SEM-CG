...
public:
	...
	*/
	static void ToolButtonPressed(Button button) {
		...
		if (button.text == "Erase") { selectedButton = 1; }
		if (button.text == "Fill") { selectedButton = 2; }
		if (button.text == "Rect") { selectedButton = 3; }
		if (button.text == "Circ") { selectedButton = 4; }
		if (button.text == "Move") { selectedButton = 5; }
		if (button.text == "Rot") { selectedButton = 6; }
		ToolEvents::Start();
	}
	...
	*/
	static void Init() {
		...
		eraseButton = Button::Create(0, 140, 78, 40, (char *)"Erase", ToolButtonPressed, true);
		...
		moveButton = Button::Create(0, 300, 78, 40, (char*)"Move", ToolButtonPressed, true);
		rotateButton = Button::Create(0, 340, 78, 40, (char*)"Rot", ToolButtonPressed, true);
	}
	...
	*/
	static void Display(int window_width, int window_height) {
		...
		eraseButton.Display(window_width, window_height);
		...
		rotateButton.Display(window_width, window_height);
	...
	*/
	static bool Pressed(int button, int state, int x, int y) {
		...
		if ((selectedButton != 1) && (eraseButton.Pressed(button, state, x, y))) {
			return true;
		}
		...
		if ((selectedButton != 5) && (moveButton.Pressed(button, state, x, y))) {
			return true;
		}
		if ((selectedButton != 6) && (rotateButton.Pressed(button, state, x, y))) {
			return true;
		}
		return false;
	}
...
void ToolEvents::Display(int window_width, int window_height) {
	...
	case 5:
		...
		break;
	case 6:
		Tool_Rotate::Display(window_width, window_height);
		break;
	case 1:
		Tool_Erase::Display(window_width, window_height);
		break;
	}
}
...
bool ToolEvents::Pressed(int button, int state, int x, int y) {
	...
	case 0:
		...
		break;
	case 1:
		if (Tool_Erase::Pressed(button, state, x, y)) {
			return true;
		}
		break;
	case 2:
	...
	case 5:
		...
		break;
	case 6:
		if (Tool_Rotate::Pressed(button, state, x, y)) {
			return true;
		}
		break;
	}
...
bool ToolEvents::SpecialKey(int key, int x, int y) {
	switch (Toolbar::selectedButton) {
	case 1:
		if (Tool_Erase::SpecialKey(key, x, y)) {
			return true;
		}
		break;
	case 5:
		...
		break;
	case 6:
		if (Tool_Rotate::SpecialKey(key, x, y)) {
			return true;
		}
		break;
	}
...
bool ToolEvents::BlockMousePress(int button, int state, int x, int y) {
	...
	case 0:
		...
		break;
	case 1:
		if (Tool_Erase::BlockMousePress(button, state, x, y)) {
			return true;
		}
		break;
	case 3:
	...
	case 5:
		...
		break;
	case 6:
		if (Tool_Rotate::BlockMousePress(button, state, x, y)) {
			return true;
		}
		break;
	}
